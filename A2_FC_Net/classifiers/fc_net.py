...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # если hidden_dims = [20, 30], то self.num_layers = 3
        self.dtype = dtype
        self.params = {}
        ...
        if type(hidden_dims) != list:
            raise ValueError('hidden_dim должен быть list')

        # Через hidden_dim функция инициализации узнает количество слоев и размерность каждого слоя 
        # количество слоев => self.num_layers = 1 + len(hidden_dims)
        # в задании скрытых слоев два [H1, H2] и их размерность 20 и 30 соответственно, т.е.:
        # input_dim = 3*32*32 = 3072
        # hidden_dims = [20, 30]; и это list
        # num_classes=10
      
        all_dims = [input_dim] + hidden_dims + [num_classes]
        # all_dims => [3072, 20, 30, 10]
        
        for layer_id in range(1, self.num_layers + 1):
            # если hidden_dims = [20, 30], то self.num_layers+1 = 4
            # all_dims[layer_id - 1] = all_dims[0] = 3072 или [input_dim]
            # последнее значение в счетчике layer_id будет 3
            # all_dims[layer_id = 3] = [num_classes]
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_out = affine_forward(current_x,
        ...
        for layer_id in range(self.num_layers - 1, 0, -1):
            ...
            grad_current_x = relu_backward(grad_current_x, caches['relu%d' % layer_id])
                                 ...
                                 # batch normalization
                                 # grad_gamma - исправить!!!!!!
            if self.normalization == 'batchnorm':
                grad_current_x, dgamma, dbeta = batchnorm_backward(grad_current_x,
                                                                           caches['batchnorm%d' % layer_id])
                grads['gamma%d' % layer_id] = dgamma
                grads['beta%d' % layer_id] = dbeta
